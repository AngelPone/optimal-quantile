...
for (scenario in c("S1", "S2", "S3", "S4")) {
  ...
  cat(total_text, file = sprintf("output/tables/simulation_%s.tex", scenario))
}
...
output <- list()
for (scenario in c("S1", "S2", "S3", "S4")) {
  table_path <- paste0("output/logs/simulation_", scenario, "_insample.csv")
  tb <- read.csv(table_path)
...
for (scenario in c("S1", "S2", "S3", "S4")) {
  ...
  cat(total_text, file = sprintf("output/tables/simulation_%s_insample.tex", scenario))
}
